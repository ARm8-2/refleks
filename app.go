...
func (a *App) LaunchKovaaksScenario(name string, mode string) (bool, string) {
	...
	deeplink := fmt.Sprintf("steam://run/%d/?action=jump-to-scenario%%3name=%s%%3mode=%s", constants.KovaaksSteamAppID, n, m)
...
func (a *App) LaunchKovaaksPlaylist(sharecode string) (bool, string) {
	...
	deeplink := fmt.Sprintf("steam://run/%d/?action=jump-to-playlist%%3sharecode=%s", constants.KovaaksSteamAppID, sc)
	runtime.BrowserOpenURL(a.ctx, deeplink)
	return true, "ok"
}
