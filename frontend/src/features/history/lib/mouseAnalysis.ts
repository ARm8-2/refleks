...
function parseEventsToKills(
  ...
  for (const event of events) {
    ...
    const m = String(event.timestamp ?? "").match(
      /^(\d{1,2}):(\d{2}):(\d{2})(?:\.(\d+))?$/,
    );
    if (!m) continue;
  ...
  for (let i = 1; i < out.length; i++) {
    if (out[i].tsAbsMs <= out[i - 1].tsAbsMs)
      out[i].tsAbsMs = out[i - 1].tsAbsMs + 1;
  }
  return out;
}
...
function analyzeWindow(
  ...
  const killPoint =
    points[Math.min(points.length - 1, endIndex)] || points[points.length - 1];
  ...
  const straight = Math.hypot(
    killPoint.x - startPoint.x,
    killPoint.y - startPoint.y,
  );
  ...
  else if (isTrackingLike) radius = clamp(Math.max(6, straight * 0.1), 6, 35);
  ...
  for (let i = startIndex; i <= endIndex; i++) {
    ...
    if (i > startIndex) {
      ...
      velocities.push(
        dt > 0 ? Math.hypot(p.x - prevP.x, p.y - prevP.y) / dt : 0,
      );
    }
  }
  ...
  const classicOvershoot =
    zoneEntryIndex !== -1 && leftZoneAfterEntry && endWithin;
  const lateEntry =
    zoneEntryIndex !== -1 && zoneEntryIndex >= distSq.length - 2;

  const maxVelocity = velocities.length > 0 ? Math.max(...velocities) : 0;
  const velocityAtKill =
  ...
  const startIndexLast300Ms = lowerBound(
    points,
    endMs - 300,
    startIndex,
    endIndex,
  ...
  for (
    let i = Math.max(1, startIndexLast300Ms - startIndex);
    i < distSq.length;
    i++
  ) {
    ...
    if (outside && previousDistSign !== 0 && sign !== previousDistSign) {
      ...
      if (
        i > 1 &&
        velocities[i - 1] !== undefined &&
        velocities[i - 2] !== undefined &&
  ...
  for (let i = Math.max(0, distSq.length - 15); i < distSq.length - 1; i++) {
    ...
    if (
      distFromTarget < radius * 3 &&
      distFromTarget > radius &&
      velocities[i] !== undefined &&
      velocities[i] < avgVelocity * 0.25
    ) {
      stallCount++;
  ...
  const hasOvershootPattern =
    classicOvershoot || (crossingCount >= 2 && hasSignificantOvershoot);
  const lateEntryGuard = !(
    lateEntry &&
    crossingCount <= 1 &&
    maxOvershootDist < radius * 1.5
  );
  const isOvershoot =
    hasOvershootPattern &&
    maxOvershootDist > Math.max(2, radius * 0.35) &&
    lateEntryGuard;
  ...
  const undershootMagnitude = hasUndershootPattern
    ? Math.max(
        avgStallDistance,
        correctionDistance / Math.max(1, correctionCount * 3),
      )
    : 0;
  const isUndershoot =
    !isOvershoot && hasUndershootPattern && undershootMagnitude > 1;
  ...
  if (isUndershoot) {
    ...
    if (undershootMagnitude < 9 || rel < 0.9 || correctionCount <= 2)
      undershootSeverity = "slight";
    else if (undershootMagnitude < 18 || rel < 1.6 || correctionCount <= 4)
      undershootSeverity = "moderate";
    else undershootSeverity = "severe";
  }

  const clickedWhileMoving =
    velocityAtKill > avgVelocity * 0.5 && velocityAtKill > 0.12;
  ...
  if (isOvershoot) {
    ...
    else if (classicOvershoot || maxOvershootDist > radius * 1.5)
      confidence += 0.15;
    ...
    if (directionFlipCount >= 4) confidence += 0.22;
      ...
      directionFlipCount >= 3 ||
      (stallCount >= 3 && correctionCount >= 2)
    )
      confidence += 0.15;
    else confidence += 0.1;
    if (directionFlipCount < 2 && correctionCount < 2) confidence -= 0.08;
  } else {
    if (efficiency > 0.85 && !leftZoneAfterEntry && crossingCount <= 1)
      confidence += 0.25;
  ...
  if (isOvershoot) {
    if (
      (!classicOvershoot && crossingCount < 3) ||
      lateEntry ||
      confidence < 0.62
    )
      overshootSeverity = downgradeSeverity(overshootSeverity);
    if (confidence < 0.45)
      overshootSeverity = downgradeSeverity(overshootSeverity);
  }
  if (isUndershoot) {
    if (
      (directionFlipCount < 3 && correctionCount < 2) ||
      (stallCount < 2 && avgStallDistance < radius * 0.4) ||
      confidence < 0.62
    )
      undershootSeverity = downgradeSeverity(undershootSeverity);
    if (confidence < 0.45)
      undershootSeverity = downgradeSeverity(undershootSeverity);
  }

  const classification: "optimal" | "overshoot" | "undershoot" = isOvershoot
    ? "overshoot"
    : isUndershoot
      ? "undershoot"
      : "optimal";
...
export function computeMouseTraceAnalysis(
  ...
  for (const ev of kills) {
    ...
    const startMs = Math.max(
      ...
      endMs -
        Math.max(
          0.1,
          Math.min(Math.max(0, ev.ttkSec) || windowCapSec, windowCapSec),
        ) *
          1000,
    );
    ...
    analyses.push(
      analyzeWindow(points, { startMs, endMs, startIndex, endIndex }, ev),
    );
  }
  ...
  let effSum = 0,
    effN = 0;
  let totalOvershootPx = 0,
    overshootKillN = 0;
  let totalUndershootPx = 0,
    undershootKillN = 0;
  ...
  for (const a of analyses) {
    if (a.classification === "overshoot") {
      ...
      overshootKillN++;
      if (a.overshootSeverity !== "none")
        severityCounts.overshoot[a.overshootSeverity]++;
    } else if (a.classification === "undershoot") {
      undershoot++;
      totalUndershootPx += a.undershootPixels;
      undershootKillN++;
      if (a.undershootSeverity !== "none")
        severityCounts.undershoot[a.undershootSeverity]++;
    } else {
      optimal++;
    }
    if (Number.isFinite(a.efficiency)) {
      effSum += a.efficiency;
      effN++;
    }
  }
  ...
  return {
    ...
    avgOvershootPixels:
      overshootKillN > 0 ? totalOvershootPx / overshootKillN : 0,
    avgUndershootPixels:
      undershootKillN > 0 ? totalUndershootPx / undershootKillN : 0,
    severityCounts,
...
export function computeSuggestedSens(
  ...
  for (const k of analysis.kills) {
    ...
    if (k.overshootPixels > 0) {
      const w =
        k.overshootSeverity === "severe"
          ? 3
          : k.overshootSeverity === "moderate"
            ? 1.5
            : 1;
    ...
    if (k.undershootPixels > 0) {
      const w =
        k.undershootSeverity === "severe"
          ? 3
          : k.undershootSeverity === "moderate"
            ? 1.5
            : 1;
  ...
  if (diff < Math.max(wOver, wUnder) * 0.25 && wOver > 5 && wUnder > 5)
    return null;

  const primaryIssue: "overshoot" | "undershoot" =
    wOver > wUnder ? "overshoot" : "undershoot";
  const avgMag =
  ...
  const base =
    severity === "severe" ? 0.2 : severity === "moderate" ? 0.12 : 0.06;
  let adj = clamp(
    base * clamp(avgMag / 15, 0.5, 1.8) * (0.6 + avgConf * 0.4),
    0.03,
    0.3,
  );
  ...
  if (primaryIssue === "overshoot") {
    ...
    const click =
      clickRate > 0.35 ? "; often clicking before fully settled" : "";
    ...
    const corr =
      avgCorrCount > 2
        ? ` with ~${fmtNum(avgCorrCount, 1)} corrections/kill`
        : "";
