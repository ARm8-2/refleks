...
import { ArrowRightLeft, ArrowUpDown, Check, ListFilter, PanelRightClose, PanelRightOpen, ScanSearch, Search, Trophy } from 'lucide-react'
import { useCallback } from 'react'
import type { RunSortKey } from '../hooks/useHistoryPageState'
import type { HistoryRun } from '../lib/historyModels'
import { formatDurationLabel, formatPercent, formatScore } from '../lib/historyModels'
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-border px-3 py-3">
        ...
        {!collapsed && (
          ...
          >
            <ScanSearch className={cn('h-4 w-4', inspectorOpen && 'text-foreground')} />
          </Button>
        )}
        <Button
          variant="ghost"
      ...
      <VirtualList
        ...
        emptyContent={
          <p className="px-2 py-6 text-center text-sm text-muted-foreground">
            {collapsed ? '—' : !session ? 'No session selected.' : 'No runs match the current search.'}
          </p>
        }
        renderItem={useCallback(
          (run: HistoryRun) => {
            ...
            const isDimmedByInspector = !!primaryRun && !isPrimary && !isCompared && run.scenarioName !== primaryRun.scenarioName
            const isDimmedByScenario = !!selectedScenario && !primaryRun && run.scenarioName !== selectedScenario
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isPrimary ? 'bg-muted font-medium' : isCompared ? 'bg-muted/60' : 'hover:bg-muted',
                    isDimmed && 'opacity-40',
                  )}
                ...
                >
                  ...
                  <span className="mt-0.5 text-[10px] leading-tight text-muted-foreground">{formatPercent(run.accuracy)}</span>
                </button>
              )
            }
            ...
            return (
              <div
                className={cn(
                  ...
                  isPrimary ? 'bg-muted' : isCompared ? 'bg-muted/60' : 'hover:bg-muted',
                  isDimmed && 'opacity-40',
                )}
              >
                <button
                ...
                <Button
                  ...
                  title={!primaryRun ? 'Select a run first' : isPrimary ? 'Primary run' : 'Compare'}
                >
          ...
          [collapsed, primaryRun, compareRun, selectedScenario, onSelectRun, onCompareRun],
        )}
      />
    </section>
  )
