...
import { buildAnalysisChartData, type AnalysisChartData, type EventsChartPoint } from '../../lib/analysisChart'
...
function formatTooltipTime(_: unknown, payload?: Array<{ payload?: { timeSec?: number } }>): string {
...
const eventsOverlayConfig: ChartConfig = {
  killsOverTime: { label: 'Pinned Kills', color: CHART_SERIES_COLORS.scoreHistory },
  accOverTime: { label: 'Pinned Acc', color: CHART_SERIES_COLORS.accuracy },
  cmpKillsOverTime: { label: 'Compare Kills', color: CHART_SERIES_COLORS.compare },
...
export function AnalysisTab({ primaryRun, compareRun, overlay }: { primaryRun: HistoryRun; compareRun: HistoryRun | null; overlay: boolean }) {
  ...
  const primaryAnalysis = useMemo(
    () => primaryEvents ? computeScenarioAnalysis(primaryRun.item.stats.summary, primaryEvents) : null,
    [primaryRun.item.stats.summary, primaryEvents],
  )
  const compareAnalysis = useMemo(
    () => compareRun && compareEvents ? computeScenarioAnalysis(compareRun.item.stats.summary, compareEvents) : null,
  ...
  if (
    ...
    || (compareRun && (compareEvents === null || comparePerformanceEvents === null))
  ) {
    return (
...
function mergeByTime(
  ...
  for (const row of b) {
    const merged: Record<string, unknown> = { timeSec: row.timeSec }
    for (const key of valueKeys) merged[`${prefix}${key[0].toUpperCase()}${key.slice(1)}`] = row[key]
    rows.push(merged)
  }

  rows.sort((left, right) => (left.timeSec as number) - (right.timeSec as number))
  return rows
}
...
function OverlayCharts({
  ...
  const eventsDomainMax = Math.max(primary.eventsDomainMax, compare.eventsDomainMax)
...
function EventsChart({ data, domainMax }: { data: EventsChartPoint[]; domainMax: number }) {
  return (
    <ChartContainer config={eventsConfig} className={`aspect-auto w-full h-full`}>
      <LineChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
        <CartesianGrid vertical={false} />
        <XAxis type="number" dataKey="timeSec" domain={[0, domainMax]} allowDataOverflow tickLine={false} axisLine={false} minTickGap={24} tickMargin={8} tickFormatter={fmtTimeTick} />
        <YAxis yAxisId="kills" tickLine={false} axisLine={false} tickMargin={8} width={40} allowDecimals={false} />
  ...
  return (
    <ChartContainer config={ttkConfig} className={`aspect-auto w-full h-full`}>
      <LineChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
        ...
        <YAxis tickLine={false} axisLine={false} tickMargin={8} width={44} tickFormatter={value => `${value}s`} />
        <ChartTooltip content={<ChartTooltipContent labelFormatter={formatTooltipTime} />} />
  ...
  return (
    <ChartContainer config={scatterConfig} className={`aspect-auto w-full h-full`}>
      <ScatterChart margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
        ...
        <YAxis type="number" dataKey="y" name="Accuracy %" tickLine={false} axisLine={false} tickMargin={8} width={44} domain={[0, 100]} tickFormatter={value => `${value}%`} />
        <ChartTooltip
          content={
            <ChartTooltipContent
              labelFormatter={() => 'Kill'}
...
function EventsChartOverlay({ data, domainMax }: { data: Array<Record<string, unknown>>; domainMax: number }) {
  return (
