...
export function SessionScenarioRadarWidget({
  ...
  const maxRuns = scenarioUsage.length > 0 ? Math.max(...scenarioUsage.map(point => point.runs)) : 1
  ...
  function renderBody(expanded: boolean) {
    ...
    if (scenarioUsage.length === 0) {
      return <EmptyState message="No scenario names found in this session yet." />
    }
    ...
    <ChartContainer config={scenarioUsageConfig} className="aspect-auto h-full w-full">
      ...
      >
        ...
        <ChartTooltip
          content={(
            <ChartTooltipContent
              labelFormatter={(_, payload) => payload?.[0]?.payload?.scenario ?? null}
            />
          )}
        />
        <Radar
